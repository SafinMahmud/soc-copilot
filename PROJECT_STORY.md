...
SOC analysts spend hours on work that is necessary but repetitive: writing SPL, running isolated searches, pivoting across auth and network logs, and manually turning raw events into something actionable—a severity rating, a timeline, MITRE mapping, and remediation steps.

We built **SOC Copilot** because we wanted to compress that workflow without sacrificing trust. The goal was never “an AI that guesses attacks.” It was an assistant that **queries Splunk first**, uses real evidence from the SIEM, and applies AI only where it adds clear value—synthesizing findings into a structured incident report analysts can review and act on.
...
**Backend (FastAPI)**  
REST endpoints: `/api/query`, `/api/investigate`, `/api/health`. Splunk access uses the official SDK on the management API. The investigation agent runs a fixed, repeatable SPL plan (auditable, not LLM-chosen). Raw findings are passed to Foundation-Sec for JSON synthesis. Deterministic fallbacks keep the app functional when the LLM is cold or unavailable.
...
To reduce investigate latency, the backend warms the Ollama VM in parallel while Splunk queries run:
...
Locally everything worked; on Cloud Run every query initially failed with `Session is not logged in`. Fixing the password secret binding and session reconnect logic was required before any live demo was credible.

**Empty data vs. convincing UI**  
When Splunk returned no events, fallback logic still showed MITRE techniques like brute force and remediation steps—with an empty or placeholder timeline. That was confusing and untrustworthy. We reworked synthesis to be evidence-driven: no Splunk rows means no attack claims.

**Timeline inconsistency**  
Foundation-Sec sometimes returned an empty timeline while Splunk had results; other runs pulled in Splunk audit noise instead of real attack events. We narrowed index filters, removed audit-index pollution from entity searches, and backfilled timelines from Splunk rows when the model omitted them.

**Cold-start latency**  
First investigate after the Ollama VM boots can take several minutes. Background VM warm-up, health checks, and a demo runbook (“warm one investigate off-camera”) made live recording practical.

**Cost vs. reliability**  
VPC connectors, always-on VMs, and GPU inference add up quickly. We used scale-to-zero Cloud Run, on-demand LLM VM start, and scheduled idle shutdown—accepting some cold-start pain to stay within a hackathon budget.
...
- A **structured analyst UX**: severity, summary, queries run, timeline, MITRE, remediation—with loading states and report navigation
